...
# Concatenation of Max_pool and Avg_pool on the channel dimension. The concatenation is also on the Channel dimension.
# Each of the elements of are size Nx1xHxW
...
class Flatten(nn.Module):  # it is like AdaptiveAvgPool2D(1) which makes it NxCx1x1 and then reshape to (size(0),-1)
    def forward(self, x):
...
class SpatialGate(nn.Module):
    ...
    def forward(self, x):   # x shape is NxCxHxW
        ...
        return masked_sp_features, torch.nn.Tanh(masked_ch_features), torch.nn.Tanh(masked_ch_features)  # Shape: NxCxHxW
