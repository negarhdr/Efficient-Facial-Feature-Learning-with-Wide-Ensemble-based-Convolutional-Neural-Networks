...
class Base(nn.Module):
    ...
    def forward(self, x):
        ...
        x_shared_representations = F.relu(self.bn4(self.conv4(x_shared_representations)))
        x_shared_representations, _ = self.cbam4(x_shared_representations)

        x_shared_representations = self.pool(F.relu(self.bn5(self.conv5(x_shared_representations))))
        x_shared_representations, _ = self.cbam5(x_shared_representations)
...
class ConvolutionalBranch(nn.Module):
    ...
    def forward_to_last_conv_layer(self, x_shared_representations):
        ...
        x_to_last_conv_layer = F.relu(self.bn5(self.conv5(x_to_last_conv_layer)))
        x_to_last_conv_layer = self.cbam5(x_to_last_conv_layer)
